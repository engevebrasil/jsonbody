...
client.on('message', async message => {
  ...
  // Se for um novo usuário, sempre inicia com a mensagem de boas-vindas
  if (!carrinhos[sender]) {
    carrinhos[sender] = {
      itens: [],
      estado: "opcoes",
  ...
  if (carrinhos[sender].estado === "confirmando_cancelamento") {
    if (text === "1") {
      // Envia mensagem personalizada de cancelamento
      await client.sendMessage(sender, 
        "🗑️ *PEDIDO CANCELADO!*\n" +
      ...
      // Apaga os dados do carrinho sem reiniciar imediatamente
      const nomeCliente = carrinhos[sender].nomeCliente;
      delete carrinhos[sender];
      
      return;
    } else if (text === "2") {
      // Mantém o carrinho e volta ao menu principal
      carrinhos[sender].estado = "opcoes";
      await client.sendMessage(sender,
        "🎉 *PEDIDO MANTIDO!\n" +
